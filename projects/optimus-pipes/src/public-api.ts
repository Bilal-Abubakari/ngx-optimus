/*
 * Public API Surface of optimus-pipes
 */

export * from "./lib/pipes/sentence-case/sentence-case.pipe";
export * from "./lib/pipes/truncate/truncate.pipe";
export * from "./lib/pipes/time-ago/time-ago.pipe";
export * from "./lib/pipes/code-case/code-case.pipe";
export * from "./lib/pipes/initials/initials.pipe";
export * from "./lib/pipes/strip-html/strip-html.pipe";
export * from "./lib/pipes/default/default.pipe";

export * from "./lib/pipes/filter-by/filter-by.pipe";

export * from "./lib/ngx-optimus-pipes.module";
